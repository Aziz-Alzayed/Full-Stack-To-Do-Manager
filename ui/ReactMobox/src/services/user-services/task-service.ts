...
import { postAuthRequest, putAuthRequest, deleteAuthRequest, getAuthRequest } from "../api-services/api-auth-service";

// Add a new task
export const addTaskApi = async (taskData: Omit<TaskModel, "id">): Promise<AxiosResponse<any, any>> => {
  try {
...
export const updateTaskApi = async (taskData: Partial<TaskModel>): Promise<AxiosResponse<any, any>> => {
  try {
...
// Delete a task by ID
export const deleteTaskApi = async (taskId: string): Promise<AxiosResponse<any, any>> => {
  try {
